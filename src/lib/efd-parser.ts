...
const RECORD_DEFINITIONS: { [key: string]: string[] } = {
  ...
  'D500': ['REG', 'IND_OPER', 'IND_EMIT', 'COD_PART', 'COD_MOD', 'COD_SIT', 'SER', 'SUB', 'NUM_DOC', 'DT_DOC', 'DT_A_P', 'VL_DOC', 'VL_ICMS', 'COD_INF', 'VL_PIS', 'VL_COFINS'],
...
export const exportRecordsToEfdText = (records: { [key: string]: EfdRecord[] }): void => {
    ...
    for (const record of finalExportList) {
        ...
        // ERROR CORRECTION 2: Ensure D500 has exactly 22 fields
        let fieldsToJoin = headers.map(field => newRecord[field] || '');
        if (reg === 'D500') {
            fieldsToJoin = fieldsToJoin.slice(0, 22);
        }
